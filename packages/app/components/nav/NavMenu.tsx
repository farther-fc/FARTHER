...
const headingStyles =
  "text-right pt-2 uppercase font-normal text-muted text-xs mt-0 mb-1 px-3 select-none";
    ...
    route.subroutes && route.subroutes.length ? (
      <DropdownMenuSub key={route.path}>
        <DropdownMenuSubTrigger
          className={cn(
            navLinkStyles,
            "data-[state=open]:bg-white/[.03] text-right mb-[2px] flex justify-between pl-1 group",
          )}
        >
          <ChevronLeft className="opacity-0 group-hover:opacity-100 group-data-[state=open]:opacity-100 transition-opacity" />
          {route.title}
        </DropdownMenuSubTrigger>
        <DropdownMenuPortal>
          <DropdownMenuSubContent
            ...
            sideOffset={4}
            // Adding an absurdly large collision padding to force it to open on the left
            collisionPadding={{ right: 600000 }}
          >
    ...
    <>
      <DropdownMenu>
        ...
        <DropdownMenuContent className="flex w-40 flex-col pt-3" align="end">
          ...
          {renderLinks(
            routesTree.filter((r) => r.type === "Info" && !r.hidden),
          )}
          <DropdownMenuSeparator />
