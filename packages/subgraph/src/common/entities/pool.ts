...
export function createPoolFees(poolAddress: Bytes, fee: i64): Bytes[] {
  ...
  // Trading Fee
  const poolTradingFee = new PoolFee(
    Bytes.fromHexString("trading-fee-").concat(poolAddress)
  );
  poolTradingFee.feeType = PoolFeeType.FIXED_TRADING_FEE;
  poolTradingFee.feePercentage = tradingFeePercentage;

  // LP Fee
  const poolLpFee = new PoolFee(
    Bytes.fromHexString("xlp-fee-").concat(poolAddress)
  );
  poolLpFee.feeType = PoolFeeType.FIXED_LP_FEE;
  ...
  // Protocol Fee
  const poolProtocolFee = new PoolFee(
    Bytes.fromHexString("xprotocol-fee-").concat(poolAddress)
  );
  poolProtocolFee.feeType = PoolFeeType.FIXED_PROTOCOL_FEE;
...
export function getPool(poolAddress: Bytes): Pool | null {
...
export function updateProtocolFees(
  ...
  const poolTradingFee = getPoolFee(
    Bytes.fromHexString("trading-fee-").concat(poolAddress)
  );
  const poolLpFee = getPoolFee(
    Bytes.fromHexString("xlp-fee-").concat(poolAddress)
  );
  const poolProtocolFee = getPoolFee(
    Bytes.fromHexString("xprotocol-fee-").concat(poolAddress)
  );
...
export function getPoolFee(id: Bytes): PoolFee {
  return PoolFee.load(id)!;
}

export function getTradingFee(poolAddress: Bytes): BigDecimal {
  const feeId = Bytes.fromHexString("trading-fee-").concat(poolAddress);
  const fee = PoolFee.load(feeId);
  if (fee === null) {
    log.warning(
      "PoolFee not found for pool: " + poolAddress.toHexString(),
      []
    );
