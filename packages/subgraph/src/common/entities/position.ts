...
export function getOrCreatePosition(
  ...
  const id = getPositionId(tokenId);
  ...
  if (position === null) {
    ...
    if (!positionCall.reverted) {
      ...
      const poolAddress = NetworkConfigs.getFactoryContract().try_getPool(
      ...
      if (poolAddress.reverted) {
        log.info("Transaction reverted", [id.toString()]);
        return null;
      }
...
export function getPositionId(tokenId: BigInt): Bytes {
  return Bytes.fromHexString("xPosition-").concatI32(tokenId.toI32());
}
