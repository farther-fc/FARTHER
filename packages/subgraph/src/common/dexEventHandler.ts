...
export class DexEventHandler {
  ...
  constructor(
    ...
    const supplyFee = getPoolFee(pool.fees[INT_ZERO]);
    const protocolFee = getPoolFee(pool.fees[INT_ONE]);
  ...
  createWithdraw(
    ...
    const pool = getPool(this.event.address);
    if (pool) {
  ...
  processLPBalanceChanges(): void {
    ...
    if (poolSnapshotDayID != this.dayID) {
      this.updateAndSavePoolDailyMetrics(poolSnapshotDayID);
      this.pool.lastSnapshotDayID = poolSnapshotDayID;
      this.pool.save();
    }
    if (poolSnapshotHourID != this.hourID) {
      this.updateAndSavePoolHourlyMetrics(poolSnapshotHourID);
  ...
  updateAndSavePoolDailyMetrics(day: i32): void {
    const id = this.event.address.concatI32(day);
    const poolMetrics = new PoolDailySnapshot(id);
    const prevPoolMetrics = PoolDailySnapshot.load(
  ...
  updateAndSavePoolHourlyMetrics(hour: i32): void {
    const id = this.event.address.concatI32(hour);
    const poolMetrics = new PoolHourlySnapshot(id);
    const prevPoolMetrics = PoolHourlySnapshot.load(
...
function getTokens(event: ethereum.Event, pool: Pool): Token[] {
  const tokens: Token[] = [];
