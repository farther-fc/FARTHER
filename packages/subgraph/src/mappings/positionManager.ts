...
import { getPool } from "../common/entities/pool";
import {
  getOrCreatePosition,
...
export function handleIncreaseLiquidity(event: IncreaseLiquidity): void {
  ...
  if (position == null) {
    log.error(
      "Position not found for IncreaseLiquidity event: {}, position: {}",
      [event.transaction.hash.toHexString(), event.params.tokenId.toString()],
    );
    return;
  }

  const pool = getPool(Address.fromBytes(position.pool));
...
export function handleDecreaseLiquidity(event: DecreaseLiquidity): void {
  ...
  if (position == null) {
    log.error(
      "Position not found for DecreaseLiquidity event: {}, position: {}",
      [event.transaction.hash.toHexString(), event.params.tokenId.toString()],
    );
    return;
  }
...
export function handleTransfer(event: Transfer): void {
  if (
    ...
    event.params.to == NetworkConfigs.getV3StakerAddress()
  ) {
    return;
  }
  ...
  // position was not able to be fetched
  if (position == null) {
    log.error("Position not found for Transfer event: {}, position: {}", [
